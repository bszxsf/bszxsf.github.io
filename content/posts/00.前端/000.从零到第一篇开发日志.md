...
本站使用[https://element-plus.org/zh-CN/](Element Plus)提供的组件，并在布局上参考了其官网的组件库文档设计：一个固定在顶部的导航栏，放置一些功能选项；下方左侧放一个侧边菜单栏，之后用于显示博文列表；右侧放主要内容。为了做到这一点，我使用`ElContainer`组件，并利用`ElAffix`将`ElHeader`固定住。
